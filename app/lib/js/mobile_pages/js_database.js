...
//Var's
var ip = '10.0.0.6';//;'192.168.0.198:8888'
var db;
var ExternalURL = 'http://'+ip+'/pfizer_app/';//;'http://'+ip+'/MAX_Local/Pfizer/IIR_Pfizer/www/' 'http://'+ip+'/Pfizer/'

//Create Db
function OpenDataBase()
{
	db = openDatabase('pfizer_alfa_taliglicerase', '1.0', '@ralves_sql', 50 * 1024 * 1024);
}
...
function handleDbSuccess(tx, error) {
	...
	*/
	...
	var sql = 
		...
		"usuariosDataChegada VARCHAR(100), " +
		"usuariosOrigem VARCHAR(100), " +
		"usuariosCia VARCHAR(100), " +
		"usuariosVoo VARCHAR(100), " +
		"usuariosHoraSaida VARCHAR(100), " +
		"usuariosHoraChegada VARCHAR(100), " +
		"usuariosDestino VARCHAR(100), " +
		"usuariosLocalizador VARCHAR(100), " +
		"usuariosEticket VARCHAR(100), " +
		"usuariosTransfer VARCHAR(100), " +
	...
	//Verificando dados da tabela: Aulas
		var sqlAulas = "select * from tb_aulas";
		tx.executeSql(sqlAulas, [], getAulas_success,handleDbError2);
	...
	//Verificando dados da tabela: Estudos
		var sqlEstudos = "select * from tb_estudos";
		tx.executeSql(sqlEstudos, [], getEstudos_success,handleDbError2);
	
	//Verificando dados da tabela: EstudosTipo
		var sqlEstudosTipo = "select * from tb_estudos_tipo";
		tx.executeSql(sqlEstudosTipo, [], getEstudosTipo_success,handleDbError2);
			...
			$.getJSON(ExternalURL+'adm/lib/php/sm_palestras.php?acao=select&format=json',function(data){
				...
				else
				{
					...
					function handleInsertDataPalestras(tx, result)
					{
						...
						tx.executeSql("INSERT INTO tb_palestras (palestrasId, palestrasTipo, palestrasTitulo, palestrasDescricao, palestrasData, palestrasHora, palestrasStatus) VALUES ("+data.mensagem[indicePalestras][0]+", "+data.mensagem[indicePalestras][1]+", '"+html_entity_decode(data.mensagem[indicePalestras][2])+"', '"+html_entity_decode(data.mensagem[indicePalestras][3])+"', '"+html_entity_decode(data.mensagem[indicePalestras][4])+"', '"+data.mensagem[indicePalestras][5]+"', 1)");
						//SALVAR NO BANCO
							console.log("Palestras: "+data.mensagem[indicePalestras][4]);
							indicePalestras++;
		...
		$.getJSON(ExternalURL+'adm/lib/php/sm_palestrantes.php?acao=select&format=json',function(data){
			...
			else
			{
				...
				function handleInsertDataPalestrantes(tx, result)
				{
					...
					//SALVAR NO BANCO
						//console.log("Palestrantes: "+data.mensagem[indicePalestrantes][4]);
						indicePalestrantes++;
