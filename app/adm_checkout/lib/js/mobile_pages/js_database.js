...
//Var's
var ip = '192.168.0.121';//;'192.168.0.198:8888'
var db;
var ExternalURL = 'http://'+ip+'/pfizer_app/app/adm_checkout/';//'http://'+ip+'/MAX_Local/Pfizer/IIR_Pfizer/www/';

//Create Db
function OpenDataBase()
{
	db = openDatabase('db_pfizer_iir', '1.0', '@ralves_sql', 50 * 1024 * 1024);
}
			...
			$.getJSON(ExternalURL+'adm/lib/php/sm_palestras.php?acao=select&format=json',function(data){
				...
				else
				{
					...
					function handleInsertDataPalestras(tx, result)
					{
						...
						tx.executeSql("INSERT INTO tb_palestras (palestrasId, palestrasTipo, palestrasTitulo, palestrasDescricao, palestrasHora, palestrasStatus) VALUES ("+data.mensagem[indicePalestras][0]+", "+data.mensagem[indicePalestras][1]+", '"+html_entity_decode(data.mensagem[indicePalestras][2])+"', '"+html_entity_decode(data.mensagem[indicePalestras][3])+"', '"+data.mensagem[indicePalestras][5]+"', 1)");
						//SALVAR NO BANCO
							//console.log("Palestras: "+data.mensagem[indicePalestras][5]);
							indicePalestras++;
		...
		$.getJSON(ExternalURL+'adm/lib/php/sm_estudos.php?acao=select&format=json',function(data){
			...
			else
			{
				...
				function handleInsertDataEstudos(tx, result)
				{
					...
					//SALVAR NO BANCO
						//console.log("Estudos: "+data.mensagem[indiceEstudos][1]);
						indiceEstudos++;
